...
TYPE2CLASS = {t:c for (c, t) in enumerate(cfg.TYPE_WHITELIST)}
print(TYPE2CLASS)
...
def extract_sunrgbd_data(idx_filename, split, output_folder, num_point=20000, type_whitelist=cfg.TYPE_WHITELIST,
        ...
        if save_votes:
            ...
            for obj in objects:
                ...
                try:
                    ...
                    pc_in_box3d,inds = box_util.extract_pc_in_box3d(\
...
def get_box3d_dim_statistics(idx_filename,
    type_whitelist=cfg.TYPE_WHITELIST,
    save_path=None):
...
if __name__=='__main__':
    ...
    if cfg.USE_V1:
        extract_sunrgbd_data(os.path.join(BASE_DIR, 'sunrgbd_trainval/train_data_idx.txt'),
            split = 'training',
            output_folder = os.path.join(BASE_DIR, 'sunrgbd_v1_pc_bbox_50k_train'),
            save_votes=False, num_point=cfg.MAX_NUM_POINT, use_v1=True, skip_empty_scene=cfg.SKIP_EMPTY_SCENE)
        extract_sunrgbd_data(os.path.join(BASE_DIR, 'sunrgbd_trainval/val_data_idx.txt'),
            split = 'training',
            output_folder = os.path.join(BASE_DIR, 'sunrgbd_v1_pc_bbox_50k_val'),
            save_votes=False, num_point=cfg.MAX_NUM_POINT, use_v1=True, skip_empty_scene=cfg.SKIP_EMPTY_SCENE)
    
    else:
        extract_sunrgbd_data(os.path.join(BASE_DIR, 'sunrgbd_trainval/train_data_idx.txt'),
            split = 'training',
            output_folder = os.path.join(BASE_DIR, 'sunrgbd_v2_pc_bbox_50k_train'),
            save_votes=False, num_point=cfg.MAX_NUM_POINT, use_v1=False, skip_empty_scene=cfg.SKIP_EMPTY_SCENE)
        extract_sunrgbd_data(os.path.join(BASE_DIR, 'sunrgbd_trainval/val_data_idx.txt'),
            split = 'training',
            output_folder = os.path.join(BASE_DIR, 'sunrgbd_v2_pc_bbox_50k_val'),
            save_votes=False, num_point=cfg.MAX_NUM_POINT, use_v1=False, skip_empty_scene=cfg.SKIP_EMPTY_SCENE)
